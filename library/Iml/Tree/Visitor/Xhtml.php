 ...
 * @license    http://creativecommons.org/licenses/by-sa/2.5/ch/     CC-By-Sa
 */
class Iml_Tree_Visitor_Xhtml implements Iml_Tree_Visitor_Interface
{
    /**
    ...
    /**
     * Holds the XHTML id attribute value
     * 
     * @var string
     */
    protected $id;
    ...
    /**
     * Wether the XHTML id attribute has been set.
     */
    private $treeIdSet;
    ...
    public function visit(Iml_Tree_Visitable_Interface $visitable)
    {
        ...
        if ($visitable instanceof Iml_Tree_Node) {
            ...
            if ($parent) {
                $this->edges[$parent->id][] = array('id' => $visitable->id, 'data' => $visitable->data); //, 'path' => $visitable->fetchPath());
            }
        }
        return true;
    }
    ...
    private function doChildren($id, $level = 0, $levelLast = array())
    {
        ...
        if ($numChildren > 0) {
            ...
            foreach ($children as $child) {
                $data = array();
                foreach ($child['data'] as $dataItem) {
                    $data[] = $this->formatData($dataItem, false);
                }
                $data = implode(' / ', $data);
                if (isset($this->edges[$child['id']])) {
                 $text.= "<li>{$data}";
                 $text.= $this->doChildren($child['id'], $level + 2, $levelLast);
